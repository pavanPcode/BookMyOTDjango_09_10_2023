...
def hospital_registration(request):
    ...
    if request.method == 'POST':
        ...
        pwd = mobile[-4:]
        data = {"inputdata": {"hospitalname" : hospitalname,"mobile" : mobile,"email" : email,"username" : email,"psw" : pwd, "tier" : None}}
        a = requests.post(url, json = data).json()
        print(a)
        if a['Status'] == False:
            if a['Message'] == 1:
                # mess = a['ErrorMessage']
                messages.error(request, "Email already exits try with another email..!")
            else:
                messages.error(request, 'Something went wrong')
        else:
            messages.success(request, "Hospital registration successfull..")
...
def authenticate_user(request):
    ...
    if request.method == 'POST':
        ...
        if response.status_code == 200 and res['Status'] == True and res['ResultData']['roleid'] == 3:
            response_data = response.json()
            user = response_data['ResultData']['username']
